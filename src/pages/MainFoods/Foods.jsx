...
function Foods({title}) {
 ...
 const { CartFoodList,setCartFoodList,filtertype ,AddFilter,setAddFilter,FoodCount,setFoodCount ,setAnimate,setAlreadyInCart} = UseMyContext( )
 ...
 const SearchFoodFunInFilter = (e) => {
  ...
  return (
     ...
     { AddFilter ? <AllFilters SearchFoodFun={SearchFoodFunInFilter}  filtertype={filtertype} isNavFilter />:<Navbar  FoodCount={FoodCount} onOpen={onOpen}  />}
    <div className="w-[90vw] max-h[300vh] flex flex-col gap-4 item-center  ">
      ...
      <div className="w-[100%] flex flex-col gap-4 px-4 mt-24 justify-center item-center">
          ...
          <p className="text-3xl font-bold font-julee text-grey">{title}s we serves...</p>
     ...
     {
                FilteredFoodData && (FilteredFoodData.filter(item=>item.type.includes(filtertype.toLowerCase()))).map((item,index)=>(
                    <FoodCard AddToCart={AddToCart}  key={index} name={item.name} text={item.text} img={item.image} price={item.price} ratings={item.ratings} id={item.id}/>
                  ))
